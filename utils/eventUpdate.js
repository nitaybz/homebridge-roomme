...
const eventUpdate = function(platform){
	...
	const roomExit = async (sensorId, roomName, userId, userName) => {
		...
		if (foundAccessory) {
			platform.log(`${roomName} RoomMe Detected Exit of ${userName}`)
			...
			// remove user from the room in cachedState
			platform.cachedState[sensorId] = _.pull(platform.cachedState[sensorId], 'user' + userId)
	...
	return (data) => {
		switch (data.event.name) {
			case 'RoomEntry':
				...
				break
			case 'RoomExit':
				if (!platform.disableExitEvents)
					roomExit(data.sensorId, data.roomName, data.userId, data.userName)
				break
			case 'LowBattery':
